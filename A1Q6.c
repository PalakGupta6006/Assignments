/*Q6. WAP to accept a coordinate point in an XY coordinate system and determine in which
quadrant the coordinate point lies.
Test Data: 7 9
Expected Output: The coordinate point (7,9) lies in the First quadrant.*/
#include<stdio.h>
int main()
{
    int x,y;
    printf("enter values of x and y coordinates: ");
    scanf("%d%d",&x,&y);
    if(x>0 && y>0)
    printf("coordinate (%d,%d)lies in first quadrant",x,y);
    else if(x<0 && y>0)
    printf("coordinate (%d,%d)lies in second quadrant",x,y);
    else if(x<0 && y<0)
    printf("coordinate (%d,%d)lies in third quadrant",x,y);
    else if(x>0 && y<0)
    printf("coordinate (%d,%d) lies in fourth quadrant",x,y);
    else
    printf("invalid coordinate");
    return 0;
}